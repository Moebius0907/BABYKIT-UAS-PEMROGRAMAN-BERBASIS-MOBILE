...
class SusuSkincareBayiPage extends StatefulWidget {
  const SusuSkincareBayiPage({super.key});

  @override
  State<SusuSkincareBayiPage> createState() => _SusuSkincareBayiPageState();
}

class _SusuSkincareBayiPageState extends State<SusuSkincareBayiPage> {
  ...
  Future<void> _initializePage() async {
    await _loadUserData();
    await _loadProductsAndFavorites();
  }
  ...
  Future<List<Product>> fetchProducts() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['success'] == true) {
        ...
        final susuSkincareProducts = productsJson
            .where(
              (p) =>
                  p['category'].toString().toLowerCase().contains('susu') ||
                  p['category'].toString().toLowerCase().contains('skincare'),
            )
            .toList();

        return susuSkincareProducts
  ...
  Future<void> _loadProductsAndFavorites() async {
    setState(() => isLoadingProducts = true);
    final products = await fetchProducts();

    if (userId.isNotEmpty && products.isNotEmpty) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data['success'] == true) {
            ...
            for (var p in products) {
              p.isFavorite = favoriteIds.contains(p.product_id);
            }
          }
        }
      } catch (e) {
  ...
  void _executeToggleFavorite(Product product, bool newStatus) async {
    if (userId.isEmpty) {
      _showSnackBar('Silakan login terlebih dahulu', Icons.error);
      return;
    }

    setState(() => product.isFavorite = newStatus);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] != true) {
          setState(() => product.isFavorite = !newStatus);
          _showSnackBar('Gagal update favorit', Icons.error);
        } else {
          _showSnackBar(
        ...
        setState(() => product.isFavorite = !newStatus);
        _showSnackBar('Gagal terhubung ke server', Icons.error);
      }
    } catch (e) {
      setState(() => product.isFavorite = !newStatus);
      _showSnackBar('Terjadi kesalahan: $e', Icons.error);
    }
  }
  ...
  Future<void> _addToCartDirectly(Product product) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['success'] == true) {
          _showSnackBar('Ditambahkan ke keranjang', Icons.shopping_cart);
        } else {
          _showSnackBar(
                                ...
                                children: [
                                  ...
                                  Positioned(
                                    ...
                                    child: Container(
                                      ...
                                      child: Text(
                                        produk.subCategory ?? produk.category,
                                        style: const TextStyle(
                                          fontSize: 10,
                                          fontWeight: FontWeight.w500,
                                        ),
